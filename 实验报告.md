...
python train_clip_openai.py --data_dir . --save_dir checkpoints_clip_openai --batch_size 8 --epochs 10 --lr 1e-5 --val_ratio 0.1 --clip_model_name "/mnt/workspace/project5-multimodal-sentiment/models--openai--clip-vit-base-patch32/snapshots/3d74acf9a28c67741b2f4f2ea7635f0aaf6f0268" --cls_hidden_dim 512 --fusion_type clip_match /early / late / attn / text_only / image_only

 python train_clip_openai.py --data_dir . --save_dir checkpoints_clip_openai --batch_size 8 --clip_model_name "/mnt/workspace/project5-multimodal-sentiment/models--openai--clip-vit-base-patch32/snapshots/3d74acf9a28c67741b2f4f2ea7635f0aaf6f0268" --cls_hidden_dim 512 --fusion_type clip_match /early / late / attn --do_infer --ckpt_path checkpoints_clip_openai/best_clip_openai.pt --output_path submission_clip_openai_early.txt


python train.py --data_dir . --save_dir checkpoints_bert_vit_big --val_ratio 0.1 --batch_size 8 --epochs 10 --lr 1e-4 --modality both --model_type bert_vit --fusion_type cross_attn --text_hidden_dim 512 --text_num_layers 8 --fusion_hidden_dim 512 --cls_hidden_dim 256




python train_vilbert.py --data_dir . --save_dir checkpoints_vilbert --batch_size 8 --epochs 10 --lr 1e-4 --val_ratio 0.1 --bert_name "/mnt/workspace/project5-multimodal-sentiment/models--bert-base-uncased/snapshots/86b5e0934494bd15c9632b12f734a8a67f723594" --vit_name "/mnt/workspace/project5-multimodal-sentiment/models--google--vit-base-patch16-224-in21k/snapshots/b4569560a39a0f1af58e3ddaf17facf20ab919b0 --hidden_dim 768 --fusion_layers 2 --fusion_heads 8 --cls_hidden_dim 512
...
预训练vilbert（attn_fuse）  211.033 M  67

预训练clip(clip_fuse) 152.328 M  74

bert+vit（attn_fuse）  125.646 M  62



直接拿多模态模型做：
early late attn clip
76     74    77     74
...
在我们现在的 image_only 设定下：

文本被置空，模型只能靠图像。
数据集中的图片很多只是配图、场景图，和情感标签的关联度本来就不高。
CLIP 预训练任务是「图文对齐」，不是「看图判断情感」。


这表明在本数据集上，情感信号主要来自文本，图像模态与情感标签的相关性较弱，且在仅凭图像的设定下模型表现甚至低于理论随机水平（33%），说明视觉信息单独使用时不仅难以提升性能，反而可能引入偏置。
